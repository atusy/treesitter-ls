...
pub use settings::{CaptureMappings, CaptureMapping, HighlightItem, HighlightSource, LanguageConfig, TreeSitterSettings, QueryTypeMappings};

/// Merge two TreeSitterSettings, preferring values from `primary` over `fallback`
pub fn merge_settings(fallback: Option<TreeSitterSettings>, primary: Option<TreeSitterSettings>) -> Option<TreeSitterSettings> {
    match (fallback, primary) {
        ...
        (Some(fallback), Some(primary)) => {
            let merged = TreeSitterSettings {
                ...
                capture_mappings: merge_capture_mappings(fallback.capture_mappings, primary.capture_mappings),
            };
            Some(merged)
        }
    }
...
mod tests {
    ...
    fn test_merge_settings_fallback_only() {
        ...
        assert_eq!(result.search_paths, Some(vec!["/path/to/fallback".to_string()]));
    }
    ...
    fn test_merge_settings_primary_only() {
        ...
        assert_eq!(result.search_paths, Some(vec!["/path/to/primary".to_string()]));
    }
    ...
    fn test_merge_settings_prefer_primary() {
        ...
        assert_eq!(result.languages["rust"].library, Some("/primary/rust.so".to_string()));
    ...
    fn test_merge_capture_mappings() {
        ...
        fallback_highlights.insert("variable.builtin".to_string(), "fallback.variable".to_string());
        fallback_highlights.insert("function.builtin".to_string(), "fallback.function".to_string());
        
        fallback_mappings.insert("_".to_string(), QueryTypeMappings {
        ...
        primary_highlights.insert("variable.builtin".to_string(), "primary.variable".to_string());
        primary_highlights.insert("type.builtin".to_string(), "primary.type".to_string());
        
        primary_mappings.insert("_".to_string(), QueryTypeMappings {
