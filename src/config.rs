...
pub use settings::{
    BridgeConfig, BridgeLanguageConfig, CaptureMapping, CaptureMappings, LanguageConfig,
    LanguageSettings, QueryTypeMappings, TreeSitterSettings, WorkspaceSettings,
};
    ...
    match (fallback, primary) {
        ...
        (Some(fallback), Some(primary)) => {
            let merged = TreeSitterSettings {
                ...
                // Prefer primary language_servers, fall back to fallback (PBI-122)
...
impl From<&WorkspaceSettings> for TreeSitterSettings {
    fn from(settings: &WorkspaceSettings) -> Self {
        ...
        TreeSitterSettings {
            ...
            bridge: settings.bridge.clone(),
            language_servers: None, // PBI-122: WorkspaceSettings doesn't track this separately
        }
    }
}
