...
use crate::analysis::{handle_code_actions, handle_selection_range, next_result_id};
...
impl LanguageServer for TreeSitterLs {
    ...
    async fn semantic_tokens_full(
        ...
        TreeSitterLs::semantic_tokens_full(self, params).await
    }
    ...
    async fn semantic_tokens_full_delta(
        ...
        TreeSitterLs::semantic_tokens_full_delta(self, params).await
    }
    ...
    async fn semantic_tokens_range(
        ...
        TreeSitterLs::semantic_tokens_range(self, params).await
    }
    ...
    async fn completion(&self, params: CompletionParams) -> Result<Option<CompletionResponse>> {
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            // Request completion with notifications capture
            let result = conn.completion_with_notifications(&virtual_uri_clone, virtual_position);
        ...
        // Handle spawn_blocking result and return connection to pool
        let (completion, notifications) = match result {
            Ok((result, conn)) => {
                self.language_server_pool.return_connection(&pool_key, conn);
                (result.response, result.notifications)
            }
    ...
    async fn references(&self, params: ReferenceParams) -> Result<Option<Vec<Location>>> {
        ...
        self.client
            .log_message(
                MessageType::INFO,
                format!(
                    "references called for {} at line {} col {}",
                    uri, position.line, position.character
                ),
            )
            .await;
        ...
        // Get the language for this document
        let Some(language_name) = self.get_language_for_document(&uri) else {
            log::debug!(target: "treesitter_ls::references", "No language detected");
            return Ok(None);
        };
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            // Request references with notifications capture
            let result = conn.references_with_notifications(
                &virtual_uri_clone,
                virtual_position,
                include_declaration,
            );
        ...
        // Handle spawn_blocking result and return connection to pool
        let (references_result, notifications) = match result {
            Ok((result, conn)) => {
                self.language_server_pool.return_connection(&pool_key, conn);
                (result.response, result.notifications)
            }
        ...
        // Translate Location ranges back to host document coordinates
        let translated_references = references_result.map(|locations| {
            locations
                .into_iter()
                .map(|loc| {
                    // Only translate if the URI matches our virtual file
                    // (references might be in other files in real scenarios)
                    let new_range = Range {
                        start: cacheable.translate_virtual_to_host(loc.range.start),
                        end: cacheable.translate_virtual_to_host(loc.range.end),
                    };
                    Location {
                        uri: uri.clone(),
                        range: new_range,
