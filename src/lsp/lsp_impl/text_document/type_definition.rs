...
impl TreeSitterLs {
    pub(crate) async fn goto_type_definition_impl(
        ...
        log::debug!(target: "treesitter_ls::type_definition", "Language: {}", language_name);
        ...
        let Some(tree) = doc.tree() else {
            log::debug!(target: "treesitter_ls::type_definition", "No parse tree");
            return Ok(None);
        };
        ...
        let Some(byte_offset) = mapper.position_to_byte(position) else {
            log::debug!(target: "treesitter_ls::type_definition", "Failed to convert position to byte");
            return Ok(None);
        };
        log::debug!(target: "treesitter_ls::type_definition", "Byte offset: {}", byte_offset);
        ...
        // Send didOpen with virtual content and wait for indexing
        ...
        // Send type_definition request and await response asynchronously
        let type_definition = conn.type_definition(virtual_position).await;

        self.client
            .log_message(
                MessageType::INFO,
                format!("Type definition response: {:?}", type_definition),
            )
            .await;

        // Translate response positions back to host document
        let Some(def_response) = type_definition else {
            self.client
                .log_message(
                    MessageType::INFO,
                    "No type definition response from rust-analyzer",
                )
                .await;
            return Ok(None);
        };
        self.client
            .log_message(
                MessageType::INFO,
                format!("Got type definition response: {:?}", def_response),
            )
            .await;
