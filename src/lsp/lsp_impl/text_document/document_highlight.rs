...
impl TreeSitterLs {
    pub(crate) async fn document_highlight_impl(
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            // Request document highlight with notifications capture
            let result =
                conn.document_highlight_with_notifications(&virtual_uri_clone, virtual_position);
        ...
        let (highlights, notifications) = match result {
            Ok((result, conn)) => {
