...
impl TreeSitterLs {
    ...
    pub(crate) async fn prepare_call_hierarchy_impl(
        ...
        let result = tokio::task::spawn_blocking(move || {
            ...
            // Request prepare call hierarchy
            let result = conn
                .prepare_call_hierarchy_with_notifications(&virtual_uri_clone, virtual_position);
    ...
    pub(crate) async fn incoming_calls_impl(
        ...
        let result = tokio::task::spawn_blocking(move || {
            let mut conn = conn;
            conn.did_open(&virtual_uri_clone, "rust", &virtual_content);
            let result = conn.incoming_calls_with_notifications(&virtual_item);
            (result, conn)
        })
        .await;

        let (calls, notifications) = match result {
            Ok((result, conn)) => {
                self.language_server_pool.return_connection(&pool_key, conn);
                (result.response, result.notifications)
            }
    ...
    pub(crate) async fn outgoing_calls_impl(
        ...
        let result = tokio::task::spawn_blocking(move || {
            let mut conn = conn;
            conn.did_open(&virtual_uri_clone, "rust", &virtual_content);
            let result = conn.outgoing_calls_with_notifications(&virtual_item);
            (result, conn)
        })
        .await;
