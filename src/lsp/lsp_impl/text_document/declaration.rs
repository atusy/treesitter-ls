...
impl TreeSitterLs {
    pub(crate) async fn goto_declaration_impl(
        ...
        log::debug!(target: "treesitter_ls::declaration", "Language: {}", language_name);
        ...
        let Some(tree) = doc.tree() else {
            log::debug!(target: "treesitter_ls::declaration", "No parse tree");
            return Ok(None);
        };
        ...
        let Some(byte_offset) = mapper.position_to_byte(position) else {
            log::debug!(target: "treesitter_ls::declaration", "Failed to convert position to byte");
            return Ok(None);
        };
        log::debug!(target: "treesitter_ls::declaration", "Byte offset: {}", byte_offset);
        ...
        self.client
            .log_message(
                MessageType::INFO,
                format!("Declaration response: {:?}", declaration),
            )
            .await;

        // Translate response positions back to host document
        let Some(decl_response) = declaration else {
            self.client
                .log_message(
                    MessageType::INFO,
                    "No declaration response from rust-analyzer",
                )
                .await;
            return Ok(None);
        };
        self.client
            .log_message(
                MessageType::INFO,
                format!("Got declaration response: {:?}", decl_response),
            )
            .await;

        // Map the response locations back to host document
        let mapped_response = match decl_response {
