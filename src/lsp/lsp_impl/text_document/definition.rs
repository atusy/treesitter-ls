...
impl TreeSitterLs {
    pub(crate) async fn goto_definition_impl(
        ...
        // Log virtual file URI for debugging
        self.client
            .log_message(
                MessageType::INFO,
                format!("Bridge virtual URI: {}", conn.virtual_file_uri()),
            )
            .await;
        ...
        self.client
            .log_message(
                MessageType::INFO,
                format!(
                    "Sending goto_definition at virtual pos line={} col={}",
                    virtual_position.line, virtual_position.character
                ),
            )
            .await;

        // Send definition request and await response asynchronously
        let definition = conn.goto_definition(virtual_position).await;
