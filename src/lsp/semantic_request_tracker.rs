...
impl SemanticRequestTracker {
    ...
    pub fn finish_request(&self, uri: &Url, request_id: u64) {
        self.active_requests.remove_if(uri, |_, info| info.id == request_id);
    }
    ...
    pub fn cleanup_stale(&self, max_age: Duration) {
        let now = Instant::now();
        self.active_requests.retain(|_, info| {
            now.duration_since(info.started_at) < max_age
        });
    }
...
mod tests {
    ...
    fn test_request_superseding() {
        ...
        let req1 = tracker.start_request(&uri);
        assert!(tracker.is_active(&uri, req1), "First request should be active");

        // Start a new request - should supersede the first
        let req2 = tracker.start_request(&uri);
        assert!(!tracker.is_active(&uri, req1), "First request should be superseded");
        assert!(tracker.is_active(&uri, req2), "Second request should be active");
    }
    ...
    fn test_multiple_uris() {
        ...
        assert!(!tracker.is_active(&uri1, req1), "Request 1 should be superseded");
        assert!(tracker.is_active(&uri2, req2), "Request 2 should still be active");
    ...
    fn test_cleanup_stale() {
        ...
        assert!(tracker.is_active(&uri, req), "Fresh request should not be cleaned up");

        // Sleep and cleanup with very short duration
        std::thread::sleep(Duration::from_millis(10));
        tracker.cleanup_stale(Duration::from_millis(5));
        assert!(!tracker.is_active(&uri, req), "Old request should be cleaned up");
    }
