...
use crate::treesitter::position_mapper::{compute_line_starts, PositionMapper};
...
impl<'a> LayerMapping<'a> {
    ...
    fn contains_doc_offset(&self, offset: usize) -> bool {
        self.doc_ranges.iter().any(|(start, end)| {
...
impl<'a> InjectionPositionMapper<'a> {
    ...
    pub fn map_to_layer_position(
        ...
        let layer_mapping = self.layers.iter()
        ...
        layer_byte_to_position(&layer_mapping.layer_text, &layer_mapping.layer_line_starts, layer_offset)
    }
...
impl<'a> PositionMapper for InjectionPositionMapper<'a> {
    fn position_to_byte(&self, position: Position) -> Option<usize> {
        doc_position_to_byte(
            self.document_text,
            &self.document_line_starts,
            position
        )
    }
...
mod tests {
    ...
    fn test_simple_injection() {
        ...
        let pos = Position { line: 2, character: 0 }; // Start of "fn main()"
    ...
    fn test_multiple_injections() {
        ...
        let pos1 = Position { line: 1, character: 0 }; // "code1"
        ...
        let pos2 = Position { line: 5, character: 0 }; // "code2"
