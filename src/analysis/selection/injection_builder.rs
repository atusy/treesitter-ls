...
use super::hierarchy_chain::chain_injected_to_host;
...
use super::range_builder::{build_selection_range, find_distinct_parent, node_to_range};
...
use super::hierarchy_chain::ranges_equal;

/// Build selection range with parsed injection content using context structs.
///
...
pub fn build_with_injection(
    ...
    let Some((hierarchy, content_node, pattern_index)) = injection_info else {
        return build_selection_range(node, doc_ctx.mapper);
    };
    ...
    let injected_selection = if let Some(nested_inj_query) = nested_injection_query.as_ref() {
            ...
            nested_injection_info
        {
            ...
            if cursor_in_nested && nested_hierarchy.len() >= 2 && inj_ctx.can_descend() {
                ...
                build_injected_selection_range(injected_node, effective_start_byte, doc_ctx.mapper)
            }
        } else {
            build_injected_selection_range(injected_node, effective_start_byte, doc_ctx.mapper)
        }
    ...
    let host_selection = Some(build_selection_range(content_node, doc_ctx.mapper));
...
/// Uses `InjectionContext` for depth tracking and parser management.
///
/// Note: This function has 9 parameters because it handles both
/// level-specific state (node, root, text, injection_query, base_language) and
/// cross-level state (doc_ctx, inj_ctx, cursor_byte, parent_start_byte).
/// The previous implementation had 11 parameters.
#[allow(clippy::too_many_arguments)]
fn build_recursive_injection(
    node: &Node,
    root: &Node,
    text: &str,
    ...
    // Increment depth - if we can't, we've hit max depth
    if inj_ctx.increment_depth().is_none() {
        return build_injected_selection_range(*node, parent_start_byte, doc_ctx.mapper);
    }
        ...
        && inj_ctx.can_descend()
    {
        ...
        build_injected_selection_range(nested_node, nested_effective_start_byte, doc_ctx.mapper)
    };
