...
fn test_query_execution_on_multiple_trees() {
    ...
    let mut parsers: Vec<Parser> = (0..3).map(|_| {
    ...
    let trees: Vec<_> = codes.iter().enumerate().map(|(i, code)| {
        parsers[i].parse(code, None).unwrap()
    }).collect();
    ...
    for (i, tree) in trees.iter().enumerate() {
        ...
        assert_eq!(match_count, 1, "Tree {} should have exactly 1 function match", i);
    }
}
...
fn test_multiple_parsers_sharing_language() {
    ...
    let mut parsers: Vec<Parser> = (0..5).map(|_| {
        let mut p = Parser::new();
        p.set_language(&language).unwrap();
        p
    }).collect();
    ...
    let codes: Vec<String> = (0..5).map(|i| {
    ...
    for (i, tree) in trees.iter().enumerate() {
        ...
        assert!(tree.root_node().child_count() > 0, "Tree {} should have children", i);
        assert!(tree.root_node().kind() == "source_file", "Tree {} root should be source_file", i);
    }
}