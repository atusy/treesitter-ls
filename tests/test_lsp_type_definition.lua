...
T["markdown"] = create_file_test_set(".md", {
	"Here is a type definition example:",
	"",
	"```rust",
	"struct MyType {", -- line 4
	"    value: i32,",
	"}",
	"",
	"fn main() {",
	"    let x: MyType = MyType { value: 42 };", -- line 9
	"}",
	"```",
})
T["markdown"]["type_definition"] = function()
	-- Position cursor on "x" variable on line 9, column 9 (on the 'x' of 'let x')
	child.cmd([[normal! 9G9|]])

	-- Verify cursor is on line 9 before type definition jump
	...
	-- Call type_definition in child vim
	child.lua([[vim.lsp.buf.type_definition()]])

	-- Poll child's cursor position until it moves to line 4 (struct definition) or timeout
	...
	MiniTest.expect.equality(
		after[1],
		4,
		("Type definition jump failed: cursor at line %d, expected line 4"):format(after[1])
	)
end
