...
async fn main() {
    ...
    let params = GotoDefinitionParams {
        text_document_position_params: TextDocumentPositionParams {
            ...
            position: Position { line: 6, character: 16 }, // "stdin" in stdin()
        },
    ...
    match result {
        Some(GotoDefinitionResponse::Scalar(location)) => {
            ...
            assert!(location.range.start.character >= 17 && location.range.start.character <= 22,
                "Should point to 'stdin' in the import, got column {}", location.range.start.character);
        }
    ...
    let params = GotoDefinitionParams {
        text_document_position_params: TextDocumentPositionParams {
            text_document: TextDocumentIdentifier { uri: uri.clone() },
            position: Position { line: 10, character: 16 }, // "stdin" in Server::new(stdin, ...)
        },
        work_done_progress_params: WorkDoneProgressParams::default(),
        partial_result_params: PartialResultParams::default(),
    };
    ...
    match result {
        Some(GotoDefinitionResponse::Scalar(location)) => {
            ...
            assert!(location.range.start.character >= 8 && location.range.start.character <= 13,
                "Should point to 'stdin' variable declaration, got column {}", location.range.start.character);
        }
