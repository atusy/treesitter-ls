...
impl TestSymbolBuilder {
    ...
    pub fn build(self) -> SymbolDefinition {
        let end_char = self.end_character.unwrap_or(self.character + self.name.len() as u32);
...
pub struct TestConfigBuilder {
    ...
    filetypes: HashMap<String, Vec<String>>,
}
...
impl TestConfigBuilder {
    ...
    pub fn with_filetypes(mut self, name: &str, extensions: Vec<&str>) -> Self {
        ...
        self.filetypes.insert(name.to_string(), extensions);
        self
    }
    
    pub fn build(self) -> TreeSitterSettings {
        TreeSitterSettings {
            treesitter: self.languages,
...
pub mod mock_data {
    ...
    pub fn multi_language_config() -> TreeSitterSettings {
        TestConfigBuilder::new()
            ...
            .with_highlights("python", vec![
                create_test_highlight_query("(function_definition) @function"),
            ])
    ...
    pub fn invalid_config_variations() -> Vec<&'static str> {
        vec![
            r#"{"invalid": "structure"}"#,
            r#"{"treesitter": {}}"#, // Missing filetypes
            r#"{"filetypes": {}}"#,  // Missing treesitter
            r#"{"treesitter": {"rust": {}}}"#, // Missing library
            r#"{"treesitter": {"rust": {"library": ""}}}"#, // Empty library
        ]
    }
}
...
pub mod assertions {
    ...
    pub fn assert_symbol_at_position(symbol: &SymbolDefinition, expected_line: u32, expected_char: u32) {
        assert_eq!(symbol.range.start.line, expected_line, 
                  "Symbol {} not at expected line", symbol.name);
        assert_eq!(symbol.range.start.character, expected_char,
                  "Symbol {} not at expected character", symbol.name);
    }
    
    pub fn assert_symbol_kind(symbol: &SymbolDefinition, expected_kind: SymbolKind) {
        assert_eq!(symbol.kind, expected_kind,
                  "Symbol {} has wrong kind", symbol.name);
    }
    ...
    pub fn assert_config_has_language(config: &TreeSitterSettings, language: &str) {
        assert!(config.treesitter.contains_key(language),
               "Config missing language: {}", language);
        assert!(config.filetypes.contains_key(language),
               "Config missing filetypes for: {}", language);
    }
    
    pub fn assert_library_path_valid(path: &str) {
        assert!(!path.is_empty(), "Library path cannot be empty");
        assert!(path.contains("tree-sitter") || path.contains("lib"),
               "Library path should contain 'tree-sitter' or 'lib': {}", path);
    }
}
...
pub mod performance {
        ...
        F: FnOnce() -> R,
    {
        ...
        assert!(duration <= max_duration,
               "{} took too long: {:?} (max: {:?})", description, duration, max_duration);
    }
...
mod helper_tests {
    ...
    fn test_config_builder() {
        ...
        assertions::assert_library_path_valid(&config.treesitter["rust"].library);
    }
