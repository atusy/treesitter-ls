# ADR-0010: Wildcard Config Inheritance
...
ADR-0009 defines how configuration merges across layers (user → project → init_options). However, there's another dimension of merging: **within a single config**, the `_` (wildcard) key serves as defaults that should be inherited by specific entries.
...
Wildcard inheritance happens **after** cross-layer merging (ADR-0009):
...
- [ADR-0009](0009-configuration-merging-strategy.md): Cross-layer configuration merging strategy
