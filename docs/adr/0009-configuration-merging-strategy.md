# ADR-0009: Configuration Merging Strategy
...
- Arrays within language config (`highlights`, `locals`, `injections`, `bridge`) are **replaced entirely**, not concatenated
- Example:
  ```toml
  # user config
  [languages.python]
  ...
  bridge = ["rust", "javascript"]                   # inherited from user
  ```

**Bridge servers HashMap** (`bridge.servers`):
...
- [ADR-0010](0010-wildcard-config-inheritance.md): Wildcard inheritance within a single config layer
