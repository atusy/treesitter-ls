...
const scrum: ScrumDashboard = {
  ...
  sprint: {
    ...
    subtasks: [
      {
        test: "Write failing test: concurrent parsing crash recovery identifies correct parser",
        implementation: "Add test simulating concurrent lua+rust parsing where rust crashes, verify only rust is marked failed",
        type: "behavioral",
        status: "completed",
        commits: [],
        notes: [
          "TDD Red: test_concurrent_parsing_crash_recovery_identifies_correct_parser",
          "Test scenario: start lua parse, start rust parse (concurrent), lua finishes, rust crashes",
          "Expected: only rust marked as failed, lua not marked as failed",
          "Test initially fails - needs end_parsing_language() API that doesn't exist yet",
        ],
      },
      {
        test: "Replace single current_parsing with per-language parsing_counts",
        implementation: "Change from Arc<ArcSwap<Option<String>>> to Arc<DashMap<String, usize>> to track concurrent parses per language",
        type: "behavioral",
        status: "completed",
        commits: [
          {
        ...
        notes: [
          "TDD Green: Replaced single current_parsing field with parsing_counts DashMap",
          "begin_parsing() increments count for language (supports concurrent calls)",
          "Added end_parsing_language() to decrement specific language count",
          "Deprecated end_parsing() for backward compatibility (clears all state)",
          "persist_state() writes all languages with count > 0 (multi-line file)",
          "init() reads multi-line crash file and marks all languages as failed",
          "Updated lsp_impl.rs to use end_parsing_language() instead of end_parsing()",
          "All 374 tests pass including new concurrent crash recovery test",
        ],
      },
      {
        test: "Verify all acceptance criteria with full test suite",
        implementation: "Run make test and make check to ensure no regressions",
        type: "behavioral",
        status: "completed",
        commits: [],
        notes: [
          "AC1: FailedParserRegistry tracks concurrent parsing operations per language - VERIFIED (DashMap<String, usize>)",
          "AC2: Parser crashes correctly identify crashing language with concurrent parses - VERIFIED (new test passes)",
          "AC3: All tests pass with concurrent-safe crash recovery - VERIFIED (374/374 tests pass)",
          "make test: 374 passed, make check: all quality checks passed",
        ],
      },
    ],
  },
