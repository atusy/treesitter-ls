...
const scrum: ScrumDashboard = {
    ...
    subtasks: [
      {
        ...
        implementation: "Add test verifying that connection eviction purges all bookkeeping state (virtual_uris, document_versions, host_to_bridge_uris, document_open_locks)",
        type: "behavioral",
        status: "completed",
        commits: [],
        notes: [
          "TDD Red: connection_eviction_purges_all_bookkeeping_state",
          "Test manually inserts mock connection state and verifies cleanup_connection_state() removes all entries",
          "Verifies cleanup of: virtual_uris (2 entries), document_versions (2 entries), host_to_bridge_uris (2 entries)",
          "Test initially fails - cleanup_connection_state() method doesn't exist yet",
        ],
      },
      {
        test: "Implement cleanup_connection_state and integrate with eviction",
        implementation: "Add cleanup_connection_state() method to purge all state for a connection key, call it from both eviction paths in get_connection()",
        type: "behavioral",
        status: "completed",
        commits: [
          {
            hash: "aebd713",
            message: "fix(bridge): prevent memory leak from orphaned bookkeeping state (PBI-169)",
            phase: "green",
          },
        ],
        notes: [
          "TDD Green: Implemented cleanup_connection_state() to purge all associated state",
          "Cleanup removes: virtual_uris (by server key), document_versions (for virtual URIs), host_to_bridge_uris (referencing virtual URIs), document_open_locks (for virtual URIs)",
          "Integrated cleanup into both eviction paths in get_connection() (lines 130 and 177)",
          "Enhanced close_documents_for_host to also clean up virtual_uris and document_open_locks (PBI-169 AC2)",
          "All 375 tests pass including new eviction cleanup test",
        ],
      },
      {
        test: "Verify all acceptance criteria with full test suite",
        implementation: "Run make test to ensure no regressions and all cleanup is complete",
        type: "behavioral",
        status: "completed",
        commits: [],
        notes: [
          "AC1: Connection eviction purges all state - VERIFIED (cleanup_connection_state called on eviction)",
          "AC2: Host document close purges virtual_uris and locks for orphaned bridge documents - VERIFIED (close_documents_for_host enhanced)",
          "AC3: All tests pass with complete bookkeeping cleanup - VERIFIED (375/375 tests pass)",
          "make test: 375 passed (added 1 new test for eviction cleanup)",
        ],
      },
    ],
  },
  ...
  // Historical sprints (recent 3) | Sprint 1-139: git log -- scrum.yaml, scrum.ts
  completed: [
    {
      number: 141,
      pbi_id: "PBI-168",
      goal: "Fix concurrent parse crash recovery to correctly identify failing parsers",
      status: "done",
      subtasks: [],
    },
    {
      ...
      subtasks: [
        ...
        },
        {
          ...
          commits: [
            {
              hash: "533230c",
              message: "fix(crash-recovery): track concurrent parsing per language (PBI-168)",
              phase: "green",
            },
          ],
          notes: [
            "Added serial_test = \"3\" to dev-dependencies in Cargo.toml (included in PBI-168 commit)",
